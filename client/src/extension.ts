...
{
    let q = vscode.window.createQuickPick();

    q.canSelectMany = true;
    q.ignoreFocusOut = true;
    q.totalSteps = 5;

    let nextStep = function(list: string[], placeHolder: string): Promise<string[]>
    {
        return new Promise<string[]>((resolve, reject) => {
            q.placeholder = placeHolder;
            q.items = list.map(x => { return { label: x, picked: x == "Россия" } });
            q.
            q.show();
            q.onDidAccept(() =>
            {
                resolve(q.selectedItems.map(x => x.label));
            });
        });        
    }
    ...
    let countryList = geoData.map(x => x.CountryName).distinct();
    let selectedCountries = await nextStep(countryList, "Выберите страну:");
    geoData = geoData.filter(x => selectedCountries.contains(x.CountryName));

    let foList = geoData.map(x => x.DistrictName).distinct();
    let selectedFO = await nextStep(foList, "Выберите ФО:");
    geoData = geoData.filter(x => selectedFO.contains(x.DistrictName));

    let strataList = geoData.map(x => x.DistrictName).distinct();
    let selectedStrata = await nextStep(strataList, "Выберите население:");
    geoData = geoData.filter(x => selectedStrata.contains(x.StrataName));
