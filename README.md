...
Из кодовых вставок в скрипте можно переходить к определению функций/переменных, описанных в Methods. Также поддерживается переход к Page и List по Id. Это делается с помощью соответствующих пунктов контекстного меню (`Перейти к определению (F12)` или `Показать определени (Alt+F12)`).
