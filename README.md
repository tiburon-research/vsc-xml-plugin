...
Для удобства обращения к XML-элементам создан специальный объект `document` класса `DocumentObjectModel`. Структуру класса можно посмотреть в файле по ссылке: [DomSignature]("file:///Z:\Разное\TiburonXMLHelper\DomSignature.d.ts")
