...
* Преобразовать в Page/Answer/Item

Преобразует выделенные строки в соответствующие элементы, с выделением Id.  
Для Items автоматически добавляется `<List>`.  
Для Page всегда генерируется полная структура: Page+Question+Answer. Первая строка считается заголовком вопроса и в начале неё ищется Id (+автоматическая транслитерация в случае русских символов).  
Если нужно сгенерировать только список Answer, то надо пользоваться преобразованием в Answers.
