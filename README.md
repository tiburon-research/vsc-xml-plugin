...
Выполняет указанный JS-код (`Ctrl` + `Alt` + `J`).
...
С помощью расширения есть возможность запускать свои JS-скрипты. Делается это с помощью команды `Выполнить скрипт` (tib.runCustomScript, `Ctrl` + `Alt` + `J`). В окне для ввода нужно ввести код и нажать клавишу ввода. Для хранения JS-методов в XML-документе надо использовать комментарий специального вида (содержимое подсвечивается как JS-синтаксис):
...
-->
```
Перед запуском команды всё содержимое таких комментариев добавляется к контексту выполнения кода, поэтому в строке ввода команда можно просто вызывать созданный метод (например `test()` из примера выше).

Для удобства обращения к XML-элементам создан специальный объект `document` класса `DocumentObjectModel`. Структуру класса можно посмотреть в файле по ссылке: [DomSignature.d.ts](file:///\\192.168.177.11\Garbage\Разное\TiburonXMLHelper\DomSignature.d.ts).
