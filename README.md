...
Для удобства обращения к XML-элементам создан специальный объект `document` класса `DocumentObjectModel`. Структуру класса можно посмотреть в файле по ссылке: [DomSignature](file:///\\192.168.177.11\Garbage\Разное\TiburonXMLHelper\DomSignature.d.ts)


Т.к. код выполняется в контексте расширения, 
