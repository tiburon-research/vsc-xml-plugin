...
* **Trusted locations**
  ...
  - запустить командную строку (`ctrl`+`shift`+`P`)
  - выполнить команду "`Workspaces: Manage Workspace Trust`"
  - и там с помощью кнопки "`Add Folder`" надо добавить необходимые вам сетевые диски
...
* `tib.largeFileLineCount` - определяет количесво строк, больше которого файл считается тяжёлым и функционал плагина ограничивается.
___
...
* Путь для материалов: после ввода `src="` расширение предлагает подставить путь `src="@ContentUrl/` с номером проекта из имени файла.
...
Если нужно сгенерировать только Answer, используйте команду `Преобразовать в Answer без Question` (`ctrl`+`alt`+`shift`+`a`).
...
Порядок:
1) Выделяем список вопросов и жмём `ctrl` + `alt` + `q` (Получть Question из выделенного текста).
2) Выделяем список ответов (возможно с заголовком) и жмём `ctrl` + `alt` + `a` (Преобразовать в Answer/Item).
3) Выбираем блок/Union/простой
...
- вместо XMLPath в строке состояния показывается надпись `TibXML: LargeFile`.
