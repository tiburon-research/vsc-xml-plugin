...
Расширение полностью переписано, чтобы не зависало при сохранении больших файлов.
...
* Из настроек удалён параметр `tib.ignoreComments`.
* В VSC пробелом вызываются все шаблоны кода (в этом расширении — это XML-заготовки), поэтому они были убраны в низ списка автозавершений (настройка `editor.snippetSuggestions`), чтобы не мешались.
* Проблемы закрытых документов больше не показываются.
