# 2.24.0 (11.01.2024)
* преобразования в XML теперь делают первую букву заглавной
* некоторые скрытые символы теперь удаляются при генерации XML из текста

# 2.23.0 (03.01.2024)
* добавлено получение шаблонов с диска Т


# 2.22.0 (17.02.2023)
* исправлены ошибки автоматической генерации XML
* настройка `tib.enableUiIsolateGen` удалена, Isolate теперь генерируется всегда.

# 2.21.0 (06.02.2023)
* Модуль разбора текста для XML стал обучаемым
* добавлена проверка `$repeat` внутри кодовых тэгов

# 2.20.0 (20.12.2022)
* Разбор текста для XML уехал в общий модуль для 2 плагинов и стал умнее

# 2.19.0 (28.11.2022)
* Модули географии и XML-объектов вынесены в общие для обоих плагинов.

# 2.17.0 (08.07.2022)
* Оптимизирован процесс получения данных скрипта.


# 2.16.0 (20.06.2022)
* Обновлены библиотеки до последних версий. Чтобы просмотр по ctrl работал.

# 2.15.0 (27.10.2021)
* Тема вынесена в отдельный плагин для более удобного перехода на плагин для SPA-двигла

# 2.14.0 (07.06.2021)
### Улучшения
* При создании географии из нескольких стран теперь создаётся список `countryList` и его Id проставляются в Var `districtList`.
* Число строк, после которого файл считается большим, теперь находится в настройках (`largeFileLineCount`).

### Исправления
* При создании географии больше нет пустых шагов.


# 2.13.0 (30.11.2020)

### Улучшения
* `@StoreUrl` во всех XML-шаблонах заменены на `@ContentUrl`.
* Переменные, атрибуты и их значения приведены к актуальному состоянию (а также добавлены в XML-шаблонах), в том числе:
  - добавлен `Manual` в Quota,
...
* Диагностика: добавлена проверка нечисловых Id в ответах.
* В шаблон `_rangeQuestions` добавлена проверка на не пустой вопрос ранжирования.

### Исправления
* Алгоритм вычленения Id ответов иногда захватывал лишние символы.


# 2.12.0 (02.10.2020)

### Улучшения
* Добавлена команда генерации блока вопросов/Union (`ctrl` + `alt` + `q` забирает список вопросов, а затем `ctrl` + `alt` + `a` забирает ответы с заголовком).
* В географии теперь можно выбрать численность населения вместо страты.
* При открытии демки удаляются все `Imperative="false"` внутри.
* Добавлен атрибут `Tech` для `Quota`.

### Исправления
* При генерации листа с Var'ами теперь Var'ы выделяются в отдельные тэги, если есть запятые внутри.
...
### Улучшения
* Сильная оптимизация (ускорение).
* Введён облегчённый режим для тяжёлых файлов, при котором отключается диагностика, а также функционал, зависящий от содержания скрипта (пользовательские методы, Id элементов и т.д.). Этот режим включается для файлов с количеством строк более 2500.
* Добавлена команда `Запустить проверку на ошибки`. Пригодится на случай, если диагностика отключена при редактировании большого документа.
* В проверку недопустимых символов добавлен символ "пространство нулевой ширины", которой может случайно попасть в скопированный из браузера текст.


# 2.10.0 (05.06.2020)

### Улучшения
* Добавлены новые двигловые методы, атрибуты и константы.
* Добавлены методы для работы с XML в пользовательских скриптах (в т.ч. добавлена работа с константами и другими документами, например, для Include).
* Добавлена команда `Документация по пользовательским скриптам`, которая открывает файл с сигнатурами.
* Добавлен шаблон для пользовательского скрипта.
* Обновлён шаблон для rangeQuestions.
* Добавлены новые проверки на: использование старого rangeQuestions, наличие C#-кода в ExportLabel
* Оптимизирована диагностика ошибок.

### Исправления
* Дублирующееся двоеточие при подстановке генерабельных MixId.
* Расширение могло влиять на документы, открытые с другой схемой (не `Tiburon XML script`).
* Индекс Var не всегда определялся правильно.
* Пустые строки при копирование элементов в виде таблицы.
* Коды вместо C#-вставок при копирование элементов в виде таблицы.
* Закомментированные части кода могли влиять на поиск парных тегов.
* Неправильная проверка на одинаковость заголовков.
* Форматирование иногда ломалось на C#-тэгах.
...
### Улучшения
* Добавлены проверки:
  - наличия `Imperative="false"`;
  - одинаковых `ExportLabel` для вопросов и ответов;
  - LINQ-проверок на `Count() > 0`;
  - Mix во вложенных в Repeat элементах (предлагает MixId);

* При генерации вопросов в Repeat добавлен `Block` для перемешивания.
* Переход к определению теперь точнее определяет объект (Page/Question/List).
* Добавлены шаблоны для Union.
* Добавлена команда `Выделить содержимое текущего тега` (`ctrl`+`k` `ctrl`+`alt`+`p`).
...
### Улучшения
* Команда преобразования XML в ListItem теперь разбивает строки по табуляции и генерирует Var. Т.е. действие, обратное команде "Скопировать как таблицу".
* Команда генерации ответов (`ctrl`+`alt`+`a`) из XML теперь адаптируется под место из которого вызывается и добавляет теги Quetion и Page на своё усмотрение. Добавление тега List при генерации Items также зависит теперь от места вставки.
* Добавлено предупреждение о многократном использовании одинаковых [c#]-вставок.
* Добавлена ошибка конфликта кавычек при c#-вставках в значениях атрибутов.
* Добавлена команда `Транслитерация текста` (`ctrl`+`k` `ctrl`+`alt`+`t`) по звучанию.
* Настройка VSCode `editor.suggest.snippetsPreventQuickSuggestions` установлена в `true`, чтобы автоподстановка констант не ломала перемещение по текущему шаблону.

### Исправления
* Автоматическая транслитерация Id вопросов теперь переводит не по звучанию, а по написанию. Т.е. русская "В" превращается не в "V", а в "B".
* В команде вставки географии теперь сразу включены Северо-Кавказский регион и страта 100-250, а также Redirect на странице обёрнут в CDATA.


# 2.7.0 (15.11.2019)

### Улучшения
* Добавлены менюшки (для ПКМ, но это на самом деле обычные команды, которые можно и вызвать через консоль, и завязать на сочетание клавиш) `Копировать как таблицу` (копирует выделенные Answer/Item в виде таблицы) и `Вынести ответы в лист` (заменяет выделенные ответы на повтор по листу и создаёт этот лист перед Page).
* Логика определения reset-ответов (при генерации XML) немного изменена: теперь Reset ищется только в последнем ответе и если текст его короче 50 символов.
* Добавлен атрибут `ExportLabel="Другое"` для сгенерированных открытых ответов.
* Добавлено предупреждение, если найдены одинаковые заголовки.

### Исправления
* Исправлены некоторые косяки подсветки синтаксиса.
* Проверка кодовых вставок в AutoSplit стала работать корректнее.


# 2.6.0 (31.10.2019)

### Улучшения
* Добавлена проверка на символ, который выдаёт себя за пробел, но таковым не является, и ломает вёрстку (появляется иногда при копировании из Excel).
* Добавлены новые и удалены старые атрибуты и константы.
* При генерации XML из текста теперь выкидываются нули в начале id ответов. А ещё теперь можно сгенерировать страницу даже если нет ответов (только заголовок вопроса); в этом случае автоматически подставляется один ответ.

### Исправления
* Исправлена подсветка $repeat с range.
* Отловлены некоторые ошибки.
* Ошибки когда-то перестали предлагать возможные исправления. Теперь снова предлагают.


# 2.5.0 (26.09.2019)

### Улучшения
* Добавлено парное переименование тега (как xml, так и html). Ставим курсор на тег и жмём F2 (команда `переименовать символ`).
* Наконец добавлен нормальный шаблон для Repeat. 3 варианта: List, Length и Range. Шаблон адаптируется под родительский тег.
* Добавлены проверки кодовых вставок в ClickText.AutoSplit.
* Константы добавлены в список автозавершения.
* Добавлен шаблон для листа взвешивания через SS (`_weightingList`).
* Удалены лишние атрибуты.

### Исправления
* При наведении теперь видны значения стандартных (встроенных в двигло) констант.
* Исправлены проблемы при форматировании C# с $-константами.

# 2.4.0 (18.09.2019)

### Улучшения
* Добавлен шаблон для последовательного ранжирования.
* Константы теперь поддерживают переход к определению и подсказки по наведению.
* Добавлена ошибка при использовании c# в ClickText.AutoSplit.
* Добавлен c#-шаблон `#ListItems`, который вставляет `CurrentSurvey.Lists[].Items.Items.Values`.
* Всякая внутренняя оптимизация работы расширения.

### Исправления
* Клиент О+К перенесён в команде `Вставить шаблон ротации` в разротированных.
* Выкинуты некоторые лишние атрибуты.
* Добавлен вариант `ничего из` для генерации Reset-ответа.
...
* При генерации Answer из текста теперь могут получится открытые и исключающие ответы, настройка `tib.enableUiIsolateGen` отвечает за добавление `<Ui Isolate="1"/>` к reset-ответу.
