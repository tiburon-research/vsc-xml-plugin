...
# 2.16.0 (20.06.2022)
* Обновлены библиотеки до последних версий. Стобы просмотр по ctrl работал.
...
* Расширение могло влиять на документы, открытые сдругой схемой (не `Tiburon XML script`).
...
* Команда генерации отетов (`ctrl`+`alt`+`a`) из XML теперь адаптируется под место из которого вызывается и добавляет теги Quetion и Page на своё усмотрение. Добавление тега List при генерации Items также зависит теперь от места вставки.
